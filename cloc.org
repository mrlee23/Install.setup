...
#+BEGIN_SRC sh
cloc [옵션] <파일|디렉토리|Git해쉬> | <set 1> <set 2> | <저장할 파일>
#+END_SRC
...
a stand{{{opt(-in)}}} for the actual file(s) to be
extracted. For example, to count lines of code
in the input files
gcc-4.2.tar.gz perl-5.8.8.tar.gz
on Unix use
{{{opt(--extract-with)}}}='gzip -dc >FILE< | tar xf -'
or, if you have GNU tar,
{{{opt(--extract-with)}}}='tar zxf >FILE<'
and on Windows use, for example:
{{{opt(--extract-with)}}}="\"c:\Program Files\WinZip\WinZip32.exe\" -e -o >FILE< ."
...
invoke 'git ls{{{opt(-files)}}}' to get a file list and
...
- {{{option(--count-and-diff)}}} SET1 SET2
...
- {{{option(--force-lang)}}}={{{argument(LANG,EXT)}}}
...
counter, use
{{{opt(--force-lang)}}}={{{argument("Fortran 90",f)}}}
...
new language filters without replacing built{{{opt(-in)}}}
...
Use {{{opt(--force-lang-def)}}} to over{{{opt(-ride)}}} cloc's
definitions (see also {{{opt(--write-lang-def)}}} ).

- {{{option(--script-lang)}}}={{{argument(LANG,S)}}}
...
and commented lines removed (in{{{opt(-line)}}} comments
...
with large files. However, these cloc{{{opt(-specific)}}}
...
- {{{option(--exclude-dir)}}}={{{argument(D1[,D2,] )}}}
...
- {{{option(--exclude-ext)}}}={{{argument(EXT1[,EXT2[...]])}}}
Do not count files having the given file name
extensions.

- {{{option(--exclude-lang)}}}={{{argument(L1[,L2,] )}}}
...
- {{{option(--include-lang)}}}={{{argument(L1[,L2,])}}}
...
- {{{option(--3)}}}
Print third{{{opt(-generation)}}} language output.
...
- {{{option(--md)}}}
Write the results as Markdown{{{opt(-formatted)}}} text.
...
#+END_EXAMPLE
 ...
 org 파일을 분류하지 못했습니다.(org 파일은 지원하지 않습니다)
...
#+END_EXAMPLE
 ...
 org 파일을 Bourne Again Shell로 분류하도록 하였습니다.(org 파일과 bash 파일은 동일한 주석처리 방법을 사용합니다)
