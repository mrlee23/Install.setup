...
#+LANG_EN: Only works for root user.
#+LANG_ES: Solo funciona para usuarios root.
#+LANG_KO: 루트 유저일 경우에만 설치 가능합니다.
#+LANG_ZH: 只能在root用户的情况下安装。
#+LANG_JA: ルートユーザである場合にのみインストール可能です。
